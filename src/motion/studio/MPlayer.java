...
//		setFrame(0);
	 ...
	 * @param frame sets the player to this frame
	 */
	public void setFrameForKeep(int frame){
		this.frame = frame;
		int frameIndex = data.animationTable[animation<<1] +frame;
		framePoolPointer = data.frameTable[(frameIndex<<2)];
	}
...
*/	
	...
	protected void drawImageClip(Graphics g,int x,int y,byte imageId,int clipIndex,byte orientation){
		...
		if(data.splitImageClips){
			Image img = ((Image[])data.imageVector.elementAt(imageId))[clipIndex-data.imageIndexTable[imageId]];
			...
			case 0:
				...
				break;
			case 1:
					g.drawRegion(img, 0, 0, img.getWidth(), img.getHeight(), Sprite.TRANS_MIRROR, xPos,yPos,20);
				break;
			case 2:
					g.drawRegion(img, 0, 0, img.getWidth(), img.getHeight(), Sprite.TRANS_MIRROR_ROT180, xPos,yPos,20);
				break;
			case 3:
					g.drawRegion(img, 0, 0, img.getWidth(), img.getHeight(), Sprite.TRANS_ROT180, xPos,yPos,20);
				break;
			}
		}
	}
