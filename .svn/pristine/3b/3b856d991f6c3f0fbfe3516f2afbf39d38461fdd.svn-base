	...
	public static MSpriteData loadMSprite(String spriteName,boolean splitImageClips,MSpriteImageLoader imageloader) throws Exception{
		...
		try{
			...
			for(int i=0;i<noOfImages;i++){
				...
				Image img = imageloader.loadImage(spriteName, i);
...
//						imageArrayForClips[j] = imageloader.loadImageClip(spriteName,i,x,y,w,h,(byte)(orientationUsedInStudio>>1)); 
						imageArrayForClips[j] = imageloader.loadImageClip(spriteName,img,x,y,w,h,0); 
					}
				}
				...
				if(splitImageClips){
					...
					data.imageVector.addElement(imageloader.loadImage(spriteName,i));
				}
			}
			// ellipse
		...
		//System.out.println("framePoolTable = "+framePoolTable.length);
